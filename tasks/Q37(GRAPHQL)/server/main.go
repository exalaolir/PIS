...
func celebrityExists(id int) (bool, error) {
	var exists bool
	err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM Celebrities WHERE id = ?)", id).Scan(&exists)
	return exists, err
}
		...
		Fields: graphql.Fields{
			"createCelebrity": &graphql.Field{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					_, err = db.Exec(
						"INSERT INTO Celebrities (id, fullname, nationality, reqPhotoPath) VALUES (?, ?, ?, ?)",
						id, fullname, nationality, reqPhotoPath,
					)
					if err != nil {
						return nil, err
			...
			"updateCelebrity": &graphql.Field{
				...
				Resolve: func(p graphql.ResolveParams) (interface{}, error) {
					...
					_, err = db.Exec(
						"UPDATE Celebrities SET fullname = ?, nationality = ?, reqPhotoPath = ? WHERE id = ?",
						fullname, nationality, reqPhotoPath, id,
					)
					if err != nil {
						return nil, err
...
func main() {
	...
	h := handler.New(&handler.Config{
		Schema:   &schema,
		Pretty:   true,
		GraphiQL: true, // Включаем GraphiQL интерфейс
	})

	http.Handle("/graphql", h)

	log.Println("GraphQL сервер запущен на http://localhost:3000/graphql")
	log.Fatal(http.ListenAndServe(":3000", nil))
}